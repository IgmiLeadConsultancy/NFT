...
const ProductEdit = () => {
  ...
  const getUserData = async () => {
    try {
      const resp = await axios.get(`http://localhost:5000/getWallets/${id}`);
 ...
 await axios({
      method: "PATCH",
      url: `http://localhost:5000/updatewallets/${id}`,
      data: JSON.stringify(newUser),
      headers: { "Content-Type": "application/json" },
    }).then((data) => {console.log(data,"Wallet Has Been Updated Successfully");
